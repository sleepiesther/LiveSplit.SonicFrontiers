...
partial class Memory {
	...
	public LazyWatcher<byte> GamePaused { get; }
	...
	internal bool Split(Settings settings) {
		...
		//Story Mode
		List<string> flagsToSplitOn = new();
		foreach (var flag in SplitBools.Where(b => !AlreadyTriggeredBools.Contains(b.Key))) {
			if (CheckBoolSplit(flag.Key, settings) && !flag.Value.Old && flag.Value.Current) {
				...
				flagsToSplitOn.Add(flag.Key);
				Log.Info("SPLIT ON " + flag.Key);
			}
		}
		//Double split protection
		if (flagsToSplitOn.Count == 1) {
			return true;
		}
		...
		// Final boss split
		if (settings.FinalBoss && LevelID.Current == SonicFrontiers.LevelID.Boss_TheEnd && EndQTECount.Old == 3 && EndQTECount.Current == 0)
			return true;

		//Another Final Boss Split
		if (LevelID.Current == SonicFrontiers.LevelID.Island_Another_Ouranos && AnotherQTECount.Old == 2 &&
			AnotherQTECount.Current == 0) {
			return true;
		}
	...
	public TimeSpan? GameTime() {
		if (GameMode.Current == SonicFrontiers.GameMode.Arcade || GameMode.Current == SonicFrontiers.GameMode.CyberspaceChallenge || GameMode.Current == SonicFrontiers.GameMode.BossRush)
			return IGT.Current + AccumulatedIGT;
		else return null;
	}
	...
	private bool CheckBoolSplit(string key, Settings settings) => key switch {
		...
		"Chaos_HackingStart" => settings.Chaos_HackingStart,
	...
	private bool ScanBossHsm(ProcessMemory process, string bossName, string bossMove) {
		...
		if (bossData.noOfElements >= 2048 || bossData.noOfElements <= 0) {
			return false;
		}
		using (ArrayRental<Address<long>> buf = new(bossData.noOfElements)) {
			...
			foreach (var entry in buf.Span) {
				...
				if (bossHsm.noOfElements >= 2048 || bossHsm.noOfElements <= 0) {
					return false;
				}
	...
	private bool playerHasComponentName(ProcessMemory process, string componentName) {
		...
		if (playerCharacter.noOfElements >= 2048 || playerCharacter.noOfElements <= 0) {
			return false;
		}
